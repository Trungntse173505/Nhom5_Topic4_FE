...
const ReviewerWorkspace = () => {
  ...
  const { taskDetail, isLoading, error, toggleAnnotationApproval } = useTaskDetail(activeTaskId);
  ...
  if (error || !taskDetail) return (
    <div className="flex h-screen bg-[#0f172a] items-center justify-center text-white flex-col gap-4">
      ...
      <button onClick={() => navigate(-1)} className="bg-blue-600 px-4 py-2 rounded">Quay lại</button>
    </div>
  );
    ...
    <div className="flex flex-col h-screen bg-[#0f172a] text-slate-200">
      <header className="flex justify-between items-center px-6 py-3 border-b border-slate-800 bg-[#1e293b]">
        <div className="flex items-center gap-4 flex-1 text-left">
          ...
          <div>
            <h1 className="text-lg font-bold text-white leading-tight">{taskDetail.taskName}</h1>
            <p className="text-xs text-slate-400">{taskDetail.projectName} • {taskDetail.taskID.substring(0, 8)}</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* CANVAS CHÍNH: Hiện ảnh và khung viền để Check */}
        ...
        {/* SIDEBAR PHẢI: Action Approve / Reject */}
