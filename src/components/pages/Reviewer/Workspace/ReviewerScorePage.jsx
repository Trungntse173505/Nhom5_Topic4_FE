...
const ReviewerCreditScorePage = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center text-white">
      <Loader2 className="animate-spin w-8 h-8 mr-3" /> Đang tải dữ liệu điểm tín nhiệm...
    </div>
  );
  ...
  if (error) return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center text-white">
      ...
      <button onClick={() => navigate('/reviewer')} className="text-purple-400 hover:text-purple-300 underline transition-colors">
        Quay lại trang chủ
      </button>
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 border-b border-slate-800 pb-6">
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <Trophy className="text-yellow-500" /> Điểm Tín Nhiệm (Reviewer)
          </h1>
          <p className="text-slate-400 mt-2">Duy trì điểm số cao để được ưu tiên phân công và nhận nhiều yêu cầu kiểm duyệt hơn.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="col-span-1 bg-[#1e293b] border border-slate-700 p-8 rounded-2xl flex flex-col items-center justify-center text-center relative overflow-hidden h-fit">
            ...
            <div className={`text-7xl font-black mb-4 z-10 ${isHigh ? 'text-purple-400' : isMed ? 'text-yellow-400' : 'text-red-500'}`}>
              {currentScore}
            </div>
            
            <div className="z-10">
              {isHigh && <span className="bg-purple-500/20 text-purple-400 border border-purple-500/30 px-4 py-2 rounded-full text-sm font-bold">Max 3 Yêu cầu / lúc</span>}
              {isMed && <span className="bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 px-4 py-2 rounded-full text-sm font-bold">Hạn chế: Max 2 Yêu cầu</span>}
              {isLow && (
                <span className="bg-red-500/20 text-red-500 border border-red-500/30 px-4 py-2 rounded-full text-sm font-bold flex items-center justify-center gap-2">
                  <ShieldAlert size={16}/> Mức cảnh báo (Max 1 Yêu cầu)
                </span>
              )}
            </div>
          </div>
          ...
          <div className="col-span-2 bg-[#1e293b] border border-slate-700 rounded-2xl p-6 flex flex-col h-full">
            ...
            {/* Vùng render danh sách */}
            <div className="space-y-3 flex-1">
              {currentLogs.length > 0 ? currentLogs.map(({ reason, createdAt, scoreChange }, index) => {
                // Chia rõ 3 trường hợp
...
export default ReviewerCreditScorePage;