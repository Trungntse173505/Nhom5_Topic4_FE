...
import { AlertCircle, MessageSquare, Clock, ArrowRight, Loader2, ShieldAlert } from 'lucide-react';

const DisputeList = () => {
  // DỮ LIỆU CỨNG (MOCK DATA) để làm UI
  const disputedTasks = [
    {
      taskID: "TASK-102",
    ...
    <div className="p-8 min-h-full bg-[#0f172a] text-slate-200">
      ...
      <div className="mb-10 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-2 text-sm font-medium">Theo dõi tiến độ giải quyết các yêu cầu phúc khảo của bạn.</p>
        </div>
        
        {/* Badge tổng số lượng */}
        <div className="bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-2xl">
          <span className="text-amber-500 font-bold">{disputedTasks.length} Đang chờ xử lý</span>
        </div>
      </div>
            ...
            >
              <div className="flex-1 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold text-white group-hover:text-amber-400 transition-colors">{task.taskName}</h3>
                </div>
                ...
                <div className="flex items-center gap-5 text-xs text-slate-500 font-medium">
                  ...
                  <div className="flex items-center gap-1.5 text-amber-500/80">
                    ...
                    <span>Trạng thái: Đang xem xét</span>
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-3 w-full md:w-auto pt-4 md:pt-0 border-t md:border-t-0 border-slate-800/50">
                <button className="flex-1 md:flex-none bg-amber-600 hover:bg-amber-500 text-white px-6 py-3 rounded-2xl text-sm font-bold transition-all shadow-lg shadow-amber-900/20 flex items-center justify-center gap-2">
